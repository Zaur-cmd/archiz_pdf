...
<head>
    ...
    <style>
        *{
        ...
        .client__info, .client__info label{
...
<main id="divToExport" class="w-doc mx-auto px-10 py-5 md:block hidden">
    <!-- Header -->
    <div class="relative">
        <div class="flex gap-8 items-end">
            <img height="170px" width="170px" src="{{URL('storage/img/menaLabs.png')}}" alt="">
            <img height="88px" width="88px" src="{{URL('storage/img/cap.png')}}" alt="">
            <h3 style="font-size: 19.5px" class="gray-text relative -bottom-3">CPS Clinical Pathology Services</h3>
        </div>
        <div class="absolute top-5 right-6">
            <img class="" id="qrHolder" height="80" width="80" src="#" alt="">
        ...
        <div class="w-6/12">
            <ul class="leading-4">
                <li class="flex client__info">
                    <label class="w-5/12" for="name">Name:</label>
                    <span class="w-6/12">{{ $user['name']}}</span>
                </li>
                <li class="flex client__info">
                    <label class="w-5/12" for="MRN">MRN:</label>
                    <span class="w-6/12">{{$user->mrn}}</span>
                </li>
                <li class="flex client__info">
                    <label class="w-5/12" for="Reference">Reference no:</label>
                    <span class="w-6/12">{{$user->reference_no}}</span>
                </li>
                <li class="flex client__info">
                    <label class="w-5/12" for="Gender">Gender:</label>
                    <span class="w-6/12">{{$user->gender}}</span>
                </li>
                <li class="flex client__info">
                    <label class="w-5/12" for="DateB">Date of birth:</label>
                    <span class="w-6/12">{{$user->birth_date}}</span>
                </li>
                <li class="flex client__info">
                    <label class="w-5/12" for="Location">Location:</label>
                    <span class="w-6/12">{{$user->location}}</span>
                </li>
            </ul>
        </div>
        <div class="flex items-end -mt-2">
            <div class="secretInfo__Ref">
                <h6 style="font-size: 14px;" class="font-bold text-md">
                    Ref By Dr
                </h6>
            </div>
            <div class="secretInfo__holder">
                <ul class="client__secretInfo">
                    <li class="flex items-start">
                        <label class="w-5/12" for="ID">Lab ID:</label>
                        <span>{{$user->lab_id}}</span>
                    </li>
                    <li class="flex items-start">
                        <label class="w-5/12" for="Sample">Sample No:</label>
                        <span>{{$user->sample_no}}</span>
                    </li>
                    <li class="flex items-start">
                        <label class="w-5/12" for="Passport">Passport No:</label>
                        <span>{{$user->passport_no}}</span>
                    </li>
                    <li class="flex items-start">
                        <label class="w-5/12" for="Reg">Reg Date:</label>
                        <span>{{date("d.m.Y h:i:s", strtotime($user->reg_date))}}</span>
                    </li>
                    <li class="flex items-start">
                        <label class="w-5/12" for="Collection">Collection Date:</label>
                        <span>{{date("d.m.Y h:i:s", strtotime($user->collection_date))}}</span>
                    </li>
                    <li class="flex items-start">
                        <label class="w-5/12" for="Reporting">Reporting Date:</label>
                        <span>{{date("d.m.Y h:i:s", strtotime($user->reporting_date))}}</span>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <!-- /Client info -->
    <!-- PCR text -->
    <h3 class="underline underline-offset-2 text-center mt-5 font-bold" style="font-size: 14px">Molecular biology</h3>
    <div class="border border-gray-600 mt-2">
        <div class="flex justify-between border-b border-gray-600 px-6">
            <h5 style="font-size: 12px" class="font-bold">Test</h5>
            <h5 style="font-size: 12px" class="font-bold">Result</h5>
            <h5 style="font-size: 12px" class="font-bold">Methodology</h5>
        </div>
        <div class="px-6">
            <div class="flex justify-between mt-4">
                <h5 class="text-xxs font-bold">COVID-19 by RT-PCR</h5>
                <h5 class="text-xxs font-bold">Not Detected (Negative)</h5>
                <h5 class="text-xxs font-medium">Muliplex Real Time PCR</h5>
            </div>
            <div style="font-size: 8.8px;" class="pl-2 text-gray-500 font-bold italic mt-2">
                <p class="mb-1">
                ...
                <ol class="list-decimal pl-3 leading-4 mb-3">
                ...
                <h4 style="font-size: 10px" class="text-center text-black font-bold">End of Repor</h4>
            </div>
        </div>
    </div>
    <!-- /PCR text -->
    ...
    <!-- Stamps -->
    <div class="flex mt-5">
        <div class="flex-[2] flex flex-col items-start justify-center font-bold text-xxs">
            <img height="80px" width="80px" src="{{URL('storage/img/signatureOnly.png')}}" alt="">
            <p class="mt-2" >
        ...
        <div class="flex-1 flex flex-col items-center">
            <img height="120px" width="120px" src="{{URL('storage/img/stamp.png')}}" alt="">
        </div>
        <div class="flex-[2] flex flex-col items-center font-bold text-xxs">
            <img height="120px" width="120px" src="{{URL('storage/img/signature.png')}}" alt="">
            <p class="mt-2" >
    ...
    <h3 style="font-size: 11px" class="uppercase text-center font-medium mt-12">This is system generated report and doest not require physicalsignature</h3>
    <p style="font-size: 9px" class="text-center font-bold">
        Al Quoz, Industrial Area 4 - P.O Box 26148, Dub ai, UAE - Tel: +971 55 538 7248 - Fax: +971 4 386 9998 - Email: <a class="text-blue-700" href="mailto: customercaredxb@menalabs.com">customercaredxb@menalabs.com</a>
    </p>
</main>
    ...
    <div class="container mx-auto px-4 py-4 mobileOnly">
        <div class="text-gray-600 font-medium">
            ...
            <ul class="leading-10">
                ...
                <li class="flex">
                    <label class="w-5/12" for="Reg">Request Date:</label>
                    <span>{{date("d.m.Y", strtotime($user->reg_date))}}</span>
                </li>
                <li class="flex">
                    <label class="w-5/12" for="Collection">Collection Date:</label>
                    <span>{{ date("d.m.Y", strtotime($user->collection_date))}}</span>
                </li>
                <li class="flex">
                    <label class="w-5/12" for="Reporting">Reporting Date:</label>
                    <span>{{date("d.m.Y", strtotime($user->reporting_date))}}</span>
                </li>
            </ul>
        </div>
